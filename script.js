...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (yearSelect && monthSelect) {
        ...
        // 텔레그램 메시지 전송 (서버리스 함수 사용)
        async function sendTelegramMessage(message) {
            try {
                console.log('텔레그램 전송 시작...');
                
                // Netlify Functions를 통한 전송 시도
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.log('✅ Netlify Functions를 통한 전송 성공:', result);
                        alert('텔레그램 전송 성공!');
                        return;
                    } else {
                        console.warn('Netlify Functions 실패, 대체 방법 시도');
                    }
                } catch (netlifyError) {
                    console.warn('Netlify Functions 오류:', netlifyError.message);
                }
                
                // GitHub Actions을 통한 전송 시도 (웹훅 방식)
                try {
                    const webhookUrl = `https://api.github.com/repos/mnj510/wake-up-check-app/dispatches`;
                    const response = await fetch(webhookUrl, {
                        method: 'POST',
                        headers: {
                            'Authorization': 'token YOUR_GITHUB_TOKEN', // 실제 토큰 필요
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({
                    ...
                    if (response.ok) {
                        console.log('✅ GitHub Actions 웹훅 전송 성공');
                        alert('텔레그램 전송 요청이 제출되었습니다!');
                        return;
                    } else {
                        console.warn('GitHub Actions 웹훅 실패, 대체 방법 시도');
                    }
                } catch (githubError) {
                    console.warn('GitHub Actions 웹훅 오류:', githubError.message);
                }
                
                // 모든 자동 전송 방법 실패 시 수동 방법 안내
                console.log('모든 자동 전송 방법 실패, 수동 방법 안내');
                ...
                if (userChoice) {
                    // 텔레그램 웹 버전 열기
                    window.open(`https://web.telegram.org/k/#@checkgetup_bot`, '_blank');
                ...
                alert('텔레그램 전송 중 오류가 발생했습니다. 수동으로 전송해주세요.');
            }
        }
